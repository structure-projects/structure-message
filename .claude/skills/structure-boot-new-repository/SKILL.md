---
name: structure-boot-new-repository
description: |
  当用户要求"新建 Repository/新增仓储/写数据访问层"时触发（structure-boot 栈）。
  按 RepositoryFacade + Delegate 模式创建仓储层。
  MUST 区分 Entity / PO，MUST 手写 toEntity / toPo。
...
when-to-use: |
  在 structure-boot 项目里新建数据访问层。
when-not-to-use: |
  - 仅修改现有 Repository（用 coding）
  - 非 DDD 形态项目（单体 4 模块用 Manager 模式）
...
stack-constraints:
  structure-boot:
    repository-package: "cn.structured.{X}.domain.repository"
    impl-package: "cn.structured.{X}.infra.repository"
    delegate-package: "cn.structured.{X}.infra.repository.delegate"
    mybatis-package: "cn.structured.{X}.repository.repository"  # ⚠️ 双 repository 是历史遗留
    base-class: "RepositoryFacade<Entity, ID, Delegate>"
    delegate-base: "MybatisPlusRepositoryDelegate<Entity, PO, ID>"
    required-methods:
      - "toEntity(PO)"
      - "toPo(Entity)"
    forbidden:
      - "Service / Controller 注入 Mapper 或 PO"
      - "直接用 @TableId 在 Entity 上"

produces:
  - {X}Repository.java（domain 接口）
  - {X}RepositoryImpl.java（infra 实现）
  - {X}RepositoryDelegate.java（infra 接口）
  - {X}MybatisPlusDelegate.java（repository-mybatis 实现）
  - {X}PO.java + {X}Mapper.java
