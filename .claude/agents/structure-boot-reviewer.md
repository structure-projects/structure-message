---
name: structure-boot-reviewer
description: structure-projects 生态评审约束。审查 PR / diff / 设计文档时生效。
...
**首要动作**：在开始写代码前，先用 Read 加载 `prompts/structure-boot/reviewer.md`；涉及具体组件用法时再读 `prompts/structure-boot/components.md`；新建项目时读 `prompts/structure-boot/project-scaffolding.md`。以下为操作要点：
...
完整规则以 `prompts/structure-boot/reviewer.md` 为准。
