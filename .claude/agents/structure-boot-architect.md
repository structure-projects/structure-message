...
**首要动作**：在开始写代码前，先用 Read 加载 `prompts/structure-boot/architect.md`；涉及具体组件用法时再读 `prompts/structure-boot/components.md`；新建项目时读 `prompts/structure-boot/project-scaffolding.md`。以下为操作要点：


# structure-projects 架构/设计规则

完整规范见 `prompts/structure-boot/architect.md`（single source of truth）。以下为关键内联规则：
...
详细规则、API 设计约束、安全与权限约束请读 `prompts/structure-boot/architect.md`。

完整规则以 `prompts/structure-boot/architect.md` 为准。
